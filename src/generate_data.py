...
Simulation logic:
  ...
  - The pipeline processes this one file and outputs weekly reports
    per client and a company report for coaching performance

Scale:
  - 40,000 clients  ->  ~960,000 food log entries for the week

Deliberate raw data imperfections (realistic, handled by processing step):
  - clients.csv has ~4% missing values in activity_level and height_cm
  - daily_calorie_target is NOT in raw data — calculated in processing
...
Produces:
  ...
  data/source/foodlog_week.csv     (~960,000 entries, this week's food logs)

Disclaimer: This script was developed with the support of Claude (Anthropic)
as an AI coding assistant. All code has been reviewed, tested, and understood
by the author.
"""
...
# Macros satisfy: protein*4 + carbs*4 + fat*9 = calories (Atwater factors).
FOOD_ITEMS = [
...
# Each meal type maps to its own food pool to ensure realistic meal composition
MEAL_FOOD_MAP = {
...
# Introduce ~2% missing height_cm (user skipped field during registration)
...
# Introduce ~2% missing activity_level (user never completed their profile)
activity_missing = np.random.choice(NUM_CLIENTS,
                                     size=int(NUM_CLIENTS * 0.02), replace=False)
clients.loc[activity_missing, "activity_level"] = np.nan

# NOTE: daily_calorie_target is intentionally absent.
# It is calculated in the processing step using the Harris-Benedict formula.
...
for day_offset in range(7):
    ...
    for user_id in client_ids:
        ...
        for meal_type in todays_meals:
            ...
            # Realistic portion variation of plus or minus 12 percent
            ...
            # For 90% of rows: calories derived from macros (always consistent)
            # protein*4 + carbs*4 + fat*9 = total calories (Atwater factors)
            cal_final = round(
                protein_final * 4 + carbs_final * 4 + fat_final * 9, 1
            )

            # For ~10% of rows: introduce calorie/macro mismatch
...
# Introduce ~1% duplicate entries (user accidentally submitted same meal twice)
...
# Sort by user and timestamp so the log reads as a natural chronological record
